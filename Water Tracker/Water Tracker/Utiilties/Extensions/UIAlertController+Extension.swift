...
//  Countdown Days
//
//  Created by Banghua Zhao on 2021/1/25.
...
//            Log("----- source")
        ...
        } else if !isIphone, let source = root, style == .actionSheet {
//            Log("----- is pad")
            popoverPresentationController?.sourceView = source
...
extension UIAlertController {
    ...
    public func show(animated: Bool = true, vibrate: Bool = false, style: UIBlurEffect.Style? = nil, completion: (() -> Void)? = nil) {
        ...
        DispatchQueue.main.async {
            UIApplication.getTopMostViewController()?.present(self, animated: animated, completion: completion)
            if vibrate {
